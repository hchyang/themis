samples.df=read.csv('samples.csv',header = T)
library(pheatmap)
library(RColorBrewer)
library(gridExtra)
library(preprocessCore)

cnv.df=read.table('CIN_matrix.txt',header = T,check.names = F)
...
fsi.df=read.table('fsi_ratio_matrix.txt',header = T)
health.v=samples.df[which(samples.df$Group=='HEALTHY'),]$Sample
coread.v=samples.df[which(samples.df$Group=='COREAD'),]$Sample
...
bin_ph<-pheatmap(bin.m,cluster_rows = F,cluster_cols = F,color = c('blue','white','red'),breaks = seq(-2,2,length.out = 4),show_rownames = F,show_colnames = F,main='Differential bins',annotation_col = annotation_col,legend = F,annotation_legend = F,cellwidth = 20,cellheight = 0.25)
